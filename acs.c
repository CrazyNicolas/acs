...
#define MAXN 535  //城市数量  表明从测试数据文件中读入多少行 
#define PI 3.141592 // Pi的定义 
#define RRR 6378.388 // 地球半径 
#define Q0 0.9//概率阈值
#define T0  9e-9// （MAXN * 最优总花费）^-1 
#define NUM_ANTS 10 //蚁群规模 
...
//局部更新规则 
void local_update(Ant *ant, int next, double *f ,int *d, double *p){
	//更新信息素矩阵
	*(f+MAXN*ant->curr+next) = (1-RHO) * (*(f+MAXN*ant->curr+next)) + RHO * T0;
	//更新信息素-距离product矩阵 
	*(p+MAXN*ant->curr+next) = 1e12*(*(f+MAXN*ant->curr+next))/pow((*(d+MAXN*ant->curr+next)),BETA);
} 

//全局更新规则 
void global_update(Ant* best, double *f, int *d, double *p){
	//先挥发 
	for(int i = 0 ; i < MAXN ; i++){
	...
	for(int i = 0 ; i < MAXN ; i++){
		for(int j = 0 ; j < MAXN ; j++){
			*(p+MAXN*i + j) = 1e12 * (*(f+MAXN*i+j))/pow((*(d+MAXN*i+j)),BETA);
		}
	} 
}

//蚂蚁向选择的城市move并且进行局部更新
void moveto(Ant *ant, int next, double *f, int *d, double *p){
	//先进行局部更新
...
int get_dis_ll(double lon1, double lat1, double lon2, double lat2){
...
//根据欧几里得距离计算
int get_dis_euc(double x1, double x2, double y1 , double y2){
...
int main(int argc, char* argv[]){
	...
	//读取所需的城市的经纬度 
	read_data("ali535.tsp",longitude,latitude);
...
//			distance[i][j] = get_dis_euc(x1,x2,y1,y2);
	...
	for(int i=0;i<MAXN;i++){
		for(int j = 0; j< MAXN; j++){
			product[i][j] = 1e12*feromine[i][j] / pow(distance[i][j],BETA);
		}
	} 
	...
	while(1){
		...
		for(int i =0 ; i < NUM_ANTS ; i++){
			...
			if(ants[i].accu_dis < Lgb){
				Lgb = ants[i].accu_dis;
				best = i; //这里用best记录下来了最好的蚂蚁的下标 
			} 
		}
		//所有蚂蚁构建完路程了 进行全局更新
		global_update(&ants[best],feromine[0],distance[0],product[0]); 
